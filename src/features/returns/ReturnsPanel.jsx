...
// Status + Type combined tab bar
const TYPE_TABS   = [{ key: "",        label: "All" }, { key: "full",    label: "Full Returns" }, { key: "partial", label: "Partial Returns" }];
const STATUS_TABS = [{ key: "",        label: "All" }, { key: "completed", label: "Completed" }, { key: "voided",    label: "Voided" }];

function TabGroup({ tabs, active, onChange, counts }) {
  return (
...
export function ReturnsPanel() {
  ...
  const { total: statTotal, fullCount, partialCount, totalRefunded } = useReturnStats();

  // Client-side search (by reference or original ref)
  const filtered = useMemo(() => {
    if (!search.trim()) return returns;
    const q = search.trim().toLowerCase();
    return returns.filter((r) =>
      r.reference_no?.toLowerCase().includes(q) ||
      r.original_ref_no?.toLowerCase().includes(q) ||
      r.customer_name?.toLowerCase().includes(q) ||
      r.cashier_name?.toLowerCase().includes(q)
    );
  }, [returns, search]);

  // Type counts for tabs (from current page data — approximate)
  const typeCounts = useMemo(() => {
    const all     = returns.length;
    const full    = returns.filter((r) => r.return_type === "full").length;
    const partial = returns.filter((r) => r.return_type === "partial").length;
  ...
  const statusCounts = useMemo(() => {
    ...
    const voided    = returns.filter((r) => r.status === "voided").length;
    return { "": all, completed, voided };
  }, [returns]);

  const handleClearFilters = useCallback(() => {
    setSearch(""); setReturnType(""); setStatus(""); setDateFrom(""); setDateTo("");
    setPage(1);
  }, []);
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-mono font-bold text-foreground">{formatRef(row.reference_no)}</p>
    ...
    },
    {
      ...
      header: "Cashier",
      render: (row) => (
        <span className="text-xs text-muted-foreground">{row.cashier_name ?? "—"}</span>
      ),
    },
    {
      key: "total_amount",
      ...
      sortable: true,
      render: (row) => (
        <span className="text-sm font-mono font-bold tabular-nums text-destructive">
          ...
          <Section
            ...
            action={
              <Button
                ...
                className={cn("h-7 gap-1.5 text-[11px]", showFilters && "text-primary")}
              >
          ...
          >
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder="Search by return ref, transaction ref, customer…"
                ...
                {hasActiveFilters && (
                  <Button variant="ghost" size="xs" onClick={handleClearFilters} className="h-8 gap-1 text-muted-foreground hover:text-foreground shrink-0">
                    <X className="h-3 w-3" />
                    Clear
                  </Button>
                )}
              ...
              {showFilters && (
                <div className="flex items-center gap-3 p-3 rounded-lg border border-border/60 bg-muted/20">
                  <span className="text-[11px] text-muted-foreground shrink-0">Date range:</span>
            ...
            <DataTable
              ...
              emptyState={
                <EmptyState
                  icon={RotateCcw}
                  title={hasActiveFilters ? "No returns match your filters" : "No returns yet"}
                  description={
                    hasActiveFilters
            ...
            {filtered.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border/40 flex items-center justify-between">
                <span className="text-[11px] text-muted-foreground">
                  ...
                  {total > filtered.length && ` of ${total} total`}
                </span>
                <div className="flex items-center gap-1.5 text-xs font-mono font-bold tabular-nums text-destructive">
